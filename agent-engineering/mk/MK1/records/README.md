...
| REC-005 | self-healing generated-code agent | generated-code execution + verifier/promotion boundary | **COVERED_BY** | REC-004 covers generated-code execution; REC-003 covers verification/gating separation. Upstream evidence remains preserved. |
| REC-006 | HR outbound messaging agent | external send + incomplete approval coverage | **COVERED_BY** | REC-002 covers dispatcher-enforced HITL positive case; REC-007 covers external-send/publication mutation. HR-specific approval gap remains preserved as UNKNOWN evidence. |
...
The self-healing example adds implementation evidence but no new MK1 dimension beyond:
...
The HR-specific approval gap remains explicit UNKNOWN evidence; it is not silently treated as safe.
...
| protocol / interoperability | REC-013 / REC-014 + protocol receipts | **COVERED / QUALIFIED for MCP and A2A classification shape** |
| modern runtime/framework | REC-014 | **COVERED** |

## Protocol coverage qualification

Protocol-family classification pressure is sufficiently represented for the current MK1 freeze candidate:

```text
MCP legacy/current revision drift      REC-013
Strands MCP 2026-07-28 modern path     systems/strands + execution receipt
Strands A2A 0.3 vs A2A 1.0 drift       systems/strands + A2A version receipt
```

A2A `1.0` compatibility for the pinned Strands implementation is **NOT ESTABLISHED**, but that missing compatibility evidence is explicit version/freshness debt rather than an unrepresentable schema gap.

## Schema pressure discovered by records

No new field has yet earned immediate promotion from the new representative records.

Two questions remain for the **schema freeze audit**:
...
Representative-record coverage fully passes only when:

1. REC-012 multi-agent gate has measured baseline/admission evidence;
2. final review confirms `COVERED_BY` decisions hide no material distinction;
3. freeze audit resolves the cross-dimension questions surfaced by the complete set;
...
A2A current-version debt is handled through explicit revision/evidence qualification and UNKNOWN routing; it must not be rewritten as a compatibility PASS.
...
- `../../../quarries/strands-a2a-version-drift.md`;
- `../../../systems/strands/`.
